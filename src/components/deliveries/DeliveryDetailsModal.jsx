    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Delivery #${delivery.id}`} size="xl">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <span className={`px-4 py-2 rounded-full text-sm font-medium ${getDeliveryStatusBadge(delivery.status)}`}>
        ...
        {/* ✅ Cancellation Reason */}
        {delivery.status === 'cancelled' && delivery.failure_reason && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mt-4">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-red-700 mt-1">{delivery.failure_reason}</p>
                {delivery.cancelled_at && (
                  <p className="text-xs text-red-600 mt-1">
                    Cancelled: {new Date(delivery.cancelled_at).toLocaleString()}
                  </p>
                )}
              </div>
            </div>
